...
and won = TRUE) / (select count(id) from Company) ,1)as percentage
from Company;
# ### B. At the company level, what is the average number of contacts that were interacted with before a successful (won) sale?
SELECT  avg(Contact1.id)
FROM Contact1
join Company on Contact1.company_id = Company.id
join Interaction on Interaction.contact_id = Contact1.id
...
select Interaction.Interaction_channel ,
   ...
   from Interaction join Contact1 on Interaction.contact_id = Contact1.id
   join Company on Contact1.company_id = Company.id 
...
from Interaction join Contact1 on Interaction.contact_id = Contact1.id join Company on Contact1.company_id = Company.id
...
*Highly accurate estimation of promotion impact along with secondary effects needs a wise mix of machine learning algorithms and coded business judgment rules. 


* The most common way to set up a lead scoring system is to use numerical values to assign a score to each lead and then categorize leads into 3–4 lead buckets according to their score.Ideally, what i think id the  target lead score has to be correlated with 10%-15% close rates. What this means is that 10%-15% of the leads that reach the Sales  team have to close in the duration of your average sales cycle.This relationship between target score and probability-to-close is important.


*Better leads mean higher conversions.Overall, predictive analytics(predictive modelling) allows you to make marketing campaign and other business decisions in a more informed manner. But as with other parts of life, predictive analytics doesn’t guarantee success.It merely increases the likelihood of success.
...
*Predictive analytics is key to successful marketing campaigns. It integrates the correlation between metrics and better business results with advanced strategies to bring more impact across the customer life cycle.

*Once the prediction models are developed, a number of optimization use cases can be supported by plugging the models into an optimization framework.


* To find out if this score is a realistic predictor i would recommend bootstrap sampling (also called bootstrapping)
 This method tests a model’s performance on certain subsets of data over and over again to provide an estimate of  accuracy.In the process you can go  back and tune parameters to simplify the model in order to improve the test performance, and then repeat the process: training and testing, training and testing.
